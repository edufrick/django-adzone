...
class AdZoneTestCase(TestCase):
    def test_model(self):
        AdZone(title="Ad Zone Title", slug="adzone", description="Ad Zone Description")
...
class AdManagerTestCase(TestCase):
    ...
    def test_get_random_ad_by_category(self):
        advert = AdBase.objects.get_random_ad("sidebar", ad_category="category-2")
...
class AdImpressionTestCase(TestCase):
    def test_model(self):
        advert = create_advert()
        AdImpression(
            impression_date=datenow(), source_ip="127.0.0.1", ad=advert,
        )
...
class AdClickTestCase(TestCase):
    def test_model(self):
        advert = create_advert()
        AdClick(
            click_date=datenow(), source_ip="127.0.0.1", ad=advert,
        )
...
class TemplateTagsTestCase(TestCase):
    ...
    def test_random_category_ad_creates_impression(self):
        create_advert()
        random_category_ad({"from_ip": "127.0.0.1"}, "sidebar", "internet-services")
        self.assertEqual(AdImpression.objects.all().count(), 1)
